...
elseif strcmp(color_option, 'mono')
    color_ls_array = repmat(palette.blue, size_grid_value, 1);    
...
q1_tick = grid_value.min(1):8*grid_value.dx(1):grid_value.max(1);
q2_tick = grid_value.min(2):8*grid_value.dx(2):grid_value.max(2);
dq1_tick = grid_value.min(3):8*grid_value.dx(3):grid_value.max(3);
dq2_tick = grid_value.min(4):8*grid_value.dx(4):grid_value.max(4);

fig = open_figure('font_size', 14, 'size', [400, 600]);

% ax = subplot(2, 2, 1);
[q1q2_unique_array, ~, ic] = unique(xs_array_in_set(:, [1, 2]), 'rows');
a_counts = accumarray(ic, 1);
color_map_counts = get_color_map([palette.yellow; palette.blue], max(a_counts)-1);
color_map_q1q2 = zeros(length(a_counts), 3);
for i = 1:length(a_counts)
    color_map_q1q2(i, :) = color_map_counts(a_counts(i), :);
end
s_q_data = scatter(q1q2_unique_array(:, 1), q1q2_unique_array(:, 2), ...
    scatter_size, color_map_q1q2, 'filled');

% s_q_data = scatter(xs_array_in_set(:, 1), xs_array_in_set(:, 2), ...
%     scatter_size, color_ls_array_in_set, 'filled');
if strcmp(color_option, 'mono')
    s_q_data.MarkerFaceAlpha = scatter_opacity;
end
hold on;
if ~isempty(gait)
...
% q2_unique_array = unique(xs_array_in_set(:, [2, 4]), 'rows');
% s_q1_data = scatter(q2_unique_array(:, 1), q2_unique_array(:, 2), ...
%     scatter_size, palette.orange, 'filled');
% s_q1_data.MarkerFaceAlpha = 0.5;
% if ~isempty(gait)
%     p_qdq_gait = plot(gait(:, 1), gait(:, 3), 'k-.');
