...
for i = istart:length(tau)
    ...
    tNow = tau(i-1);
    %% If time to reach, do that

if isfield(extraArgs, 'computeTTR') && extraArgs.computeTTR
    if i > istart
        schemeData.ttrLast = schemeData.ttr;
    else
        schemeData.ttrLast = ones(size(data0(:))).*inf;
    end
    [ y, schemeData ] = feval(@postTimestepTTR, tNow, y, schemeData);
    data = reshape(y, g.shape);
    changeTTR = max(abs(schemeData.ttr(:)-schemeData.ttrLast(:)));
end
    ...
    while tNow < tau(i) - small
        ...
        if ~isfield(extraArgs, 'discountMode') || ...
            ...
            elseif strcmp(compMethod, 'maxCBF')
                dt = tNow - tPrev;
                y = min(y ./(1-schemeData.cbf.gamma*dt), targets(:));                
            else
    ...
    %% If commanded, stop the reachable set computation once the ttr has
    % converged
    if stopConvergeTTR && changeTTR < convergeThresholdTTR
        extraOuts.stoptauTTR = tau(i);
        tau(i+1:end) = [];
...
if isfield(extraArgs, 'computeTTR') && extraArgs.computeTTR
    extraOuts.ttr = reshape(schemeData.ttr, g.shape);
    unreached = find(isinf(extraOuts.ttr));
    extraOuts.ttr(unreached) = NaN;
end
